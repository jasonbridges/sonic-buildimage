...
main() {
    ...
    if [ "$docker_inram" = "on" ]; then
        ...
        if unzip -l "$INSTALLER_PAYLOAD" "$FILESYSTEM_DOCKERFS" >/dev/null 2>&1; then
            ...
            unzip -op "$INSTALLER_PAYLOAD" "$FILESYSTEM_DOCKERFS" | zstd -dc | tar x $TAR_EXTRA_OPTION -f - -C "$demo_mnt/$image_dir/$DOCKERFS_DIR"
        fi
    fi

    if unzip -l "$INSTALLER_PAYLOAD" "platform.tar.gz" >/dev/null 2>&1; then
        mkdir -p "$demo_mnt/$image_dir/platform"
        unzip -op "$INSTALLER_PAYLOAD" "platform.tar.gz" | zstd -dc | tar x $TAR_EXTRA_OPTION -f - -C "$demo_mnt/$image_dir/platform"
    fi
