...
if [ "$IMAGE_DISTRO" = "trixie" ]; then
    DOCKER_VERSION="5:28.5.2-1~debian.13~trixie"
    CONTAINERD_IO_VERSION="1.7.28-2~debian.13~trixie"
else
    DOCKER_VERSION="5:28.5.2-1~debian.12~bookworm"
    CONTAINERD_IO_VERSION="1.7.28-2~debian.12~bookworm"
fi
...
sudo LANG=C chroot $FILESYSTEM_ROOT apt-get -y install busybox linux-base kmod acl
if [[ $CONFIGURED_ARCH == amd64 ]]; then
    sudo LANG=C DEBIAN_FRONTEND=noninteractive chroot $FILESYSTEM_ROOT apt-get -y install hdparm
fi

## Update initramfs for booting with squashfs+overlay
sudo mkdir -p $FILESYSTEM_ROOT/etc/initramfs-tools
cat files/initramfs-tools/modules | sudo tee -a $FILESYSTEM_ROOT/etc/initramfs-tools/modules > /dev/null

## Install kbuild for sign-file into docker image (not fsroot)
sudo LANG=C DEBIAN_FRONTEND=noninteractive dpkg -i --force-depends ./target/debs/${BLDENV:-bookworm}/linux-kbuild-${LINUX_KERNEL_VERSION}*_${CONFIGURED_ARCH}.deb
## Hook into initramfs: change fs type from vfat to ext4 on arista switches
sudo mkdir -p $FILESYSTEM_ROOT/etc/initramfs-tools/scripts/init-premount/ $FILESYSTEM_ROOT/etc/initramfs-tools/hooks/ $FILESYSTEM_ROOT/etc/initramfs-tools/scripts/init-bottom/
...
if [ -d "$FILESYSTEM_ROOT/usr/share/initramfs-tools/scripts/init-bottom" ]; then
    pushd "$FILESYSTEM_ROOT/usr/share/initramfs-tools/scripts/init-bottom" >/dev/null
    if ! patch -R -p1 -s --dry-run < "$OLDPWD/files/initramfs-tools/udev.patch" >/dev/null 2>&1; then
        sudo patch -N -p1 < "$OLDPWD/files/initramfs-tools/udev.patch"
    fi
    popd >/dev/null
fi
...
TZDATA_PKG=""
if [ "$IMAGE_DISTRO" = "trixie" ]; then
    FIRMWARE_MISC="firmware-intel-misc"
    TZDATA_PKG="tzdata-legacy"
fi

if [ "$CONFIGURED_ARCH" = "amd64" ]; then
    ## Pre-install hardware drivers
    sudo LANG=C chroot $FILESYSTEM_ROOT apt-get -y install \
        firmware-linux-nonfree $FIRMWARE_MISC
fi
...
if [[ $TARGET_BOOTLOADER == grub ]]; then
	...
	for deb in $basename_deb_packages; do sudo rm -f $FILESYSTEM_ROOT/${deb#./}; done
    fi
    sudo LANG=C DEBIAN_FRONTEND=noninteractive chroot $FILESYSTEM_ROOT ln -sf /lib/systemd/system/initrd.target lib/systemd/system/default.target
fi
...
if [[ $TARGET_BOOTLOADER == grub ]]; then
    ...
    sudo mkdir -p $FILESYSTEM_ROOT/$PLATFORM_DIR/grub
...
sudo LANG=C chroot $FILESYSTEM_ROOT /bin/bash -c "for f in linux-run-hooks linux-update-symlinks linux-check-removal depmod; do echo '#!/bin/sh' > /usr/bin/\$f && chmod +x /usr/bin/\$f; done"
...
sudo LANG=C DEBIAN_FRONTEND=noninteractive chroot $FILESYSTEM_ROOT dpkg -i --force-depends $basename_deb_packages
